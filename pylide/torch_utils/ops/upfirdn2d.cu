...
template <class T> static __global__ void upfirdn2d_kernel_large(upfirdn2d_kernel_params p)
{
    ...
    int inY = floor_div(midY, p.up.y);
    int h = floor_div(midY + p.filterSize.y, p.up.y) - inY;

    // Non-mirror pad: clamp to valid range (implicit zeros outside)
    if (!p.mirrorPad)
    {
        inY = min(max(floor_div(midY, p.up.y), 0), p.inSize.y);
        h = min(max(floor_div(midY + p.filterSize.y, p.up.y), 0), p.inSize.y) - inY;
    }
    ...
    for (int minorIdx = 0, minor = minorBase; minorIdx < p.loopMinor & minor < p.sizeMinor; minorIdx++, minor += p.launchMinor)
    {
        ...
        for (int loopX = 0, outX = outXBase; loopX < p.loopX & outX < p.outSize.x; loopX++, outX += blockDim.y)
        {
            ...
            int inX = floor_div(midX, p.up.x);
            int w = floor_div(midX + p.filterSize.x, p.up.x) - inX;
            
            // Non-mirror pad: clamp to valid range (implicit zeros outside)
            if (!p.mirrorPad)
            {
                inX = min(max(floor_div(midX, p.up.x), 0), p.inSize.x);
                w = min(max(floor_div(midX + p.filterSize.x, p.up.x), 0), p.inSize.x) - inX;
            }
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    // Mirror pad:
                    if (p.mirrorPad) {
                        coordX = coordX < 0 ? (-coordX) : coordX;
                        coordY = coordY < 0 ? (-coordY) : coordY;
                        int offs = floor_div(2, p.up.x); // 1x: 2, 2x: 1, 4x: ?
                        coordX = (coordX >= p.inSize.x) ? (2*p.inSize.x - coordX - offs) : coordX;
                        coordY = (coordY >= p.inSize.y) ? (2*p.inSize.y - coordY - offs) : coordY;
                    }

                    const T xp = ((const T*)p.x)[coordX * p.inStride.x + coordY * p.inStride.y + c * p.inStride.z + n * p.inStride.w];
                    const float fp = p.f[filterX * p.filterStride.x + filterY * p.filterStride.y + x * filterStepX + y * filterStepY];
...
{
    // Spec: kernel, tileOutW, tileOutH, loopMinor, loopX
